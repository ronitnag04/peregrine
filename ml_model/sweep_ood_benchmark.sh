...
SWEEP_DIR="sweep_results/ood_spec_v3"
mkdir -p "$SWEEP_DIR"
...
for benchmark in "${BENCHMARKS[@]}"; do
    ...
    if python "$PYTHON_SCRIPT" -d "$DATASET_PATH" --ood-benchmark "$benchmark" --output-dir "$OUTPUT_DIR" > "$RUN_DIR/training_output.txt" 2>&1; then
