...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train the Peregrine ML model with an OOD-benchmark split using PyTorch and XLA with the AWS Neuron SDK.")
    parser.add_argument(
        "-d",
        "--dataset-path",
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="OOD benchmark to use for testing (unseen in training unless --ood-train-size > 0)",
    )
    parser.add_argument(
        "--ood-train-size",
        type=int,
        help="Number of OOD data points to inject into the training set",
        default=0,
    )
    parser.add_argument(
        "-o",
...
def main() -> None:
    ...
    benchmarks = ids["benchmark"].to_numpy()
    ood_mask = benchmarks == args.ood_benchmark
    ood_idx = np.flatnonzero(ood_mask)
    train_idx = np.flatnonzero(~ood_mask)

    if ood_idx.size == 0:
        raise ValueError(f"No rows found for OOD benchmark {args.ood_benchmark!r}")

    if args.ood_train_size > 0:
        if args.ood_train_size >= ood_idx.size:
            raise ValueError(
                f"--ood-train-size ({args.ood_train_size}) must be smaller than "
                f"the number of OOD rows ({ood_idx.size})"
            )
        rng = np.random.default_rng(42)
        picked = rng.choice(ood_idx.size, size=args.ood_train_size, replace=False)
        keep_mask = np.ones(ood_idx.size, dtype=bool)
        keep_mask[picked] = False
        train_idx = np.concatenate([train_idx, ood_idx[picked]])
        test_idx = ood_idx[keep_mask]
    else:
        test_idx = ood_idx

    # Keep indices sorted so memmap fancy indexing stays sequential.
    ...
    print(f"Final split: {len(train_idx)} train, {len(test_idx)} test ({len(test_idx)/total:.2%} test)")
