...
for test_size in "${TEST_SIZES[@]}"; do
    ...
    # Create subdirectory for this test size
    RUN_DIR="$SWEEP_DIR/test_size_${test_size}"
    mkdir -p "$RUN_DIR"
    
    # Run training and capture output
    if python "$PYTHON_SCRIPT" -d "$DATASET_PATH" --test-size "$test_size" --output-dir "$RUN_DIR" > "$RUN_DIR/training_output.txt" 2>&1; then
...
for test_size in "${TEST_SIZES[@]}"; do
    ...
    if [ -f "$RUN_DIR/training_output.txt" ]; then
        ...
        if [ -f "$RUN_DIR/training_metrics/ronamol_spec_training_data_metrics.json" ]; then
            echo "  Metrics file: Available" >> "$SUMMARY_FILE"
        fi
        echo "" >> "$SUMMARY_FILE"
    fi
done
