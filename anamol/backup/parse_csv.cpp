...
mem_access_t parse_mem_address(const std::string& mem_str) {
  ...
  if (close_paren != std::string::npos) {
    std::string size_str =
        trim(mem_str.substr(paren_pos + 1, close_paren - paren_pos - 1));
...
instr_trace_t parse_csv_line(const std::string& line) {
  instr_trace_t inst;
...
std::vector<instr_trace_t> parse_trace_csv(const std::string& filename) {
  std::vector<instr_trace_t> instructions;
  ...
  while (std::getline(file, line)) {
    ...
    // Parse the line
    instr_trace_t inst = parse_csv_line(line);
...
void print_usage(const char* program_name) {
  std::cout << "Usage: " << program_name << " [CSV_FILE] [OPTIONS]"
            << std::endl;
  std::cout << "\nOptions:" << std::endl;
  std::cout << "  --range START END     Print instructions from START to END "
               "(inclusive)"
            << std::endl;
  std::cout << "  --start START --num NUM   Print NUM instructions starting "
               "from START"
            << std::endl;
  std::cout << "\nExamples:" << std::endl;
  std::cout << "  " << program_name << " trace.csv --range 0 10" << std::endl;
  std::cout << "  " << program_name << " trace.csv --start 100 --num 5"
            << std::endl;
  std::cout << "  " << program_name
            << " trace.csv  (prints first 5 instructions by default)"
            << std::endl;
}

// Print a single instruction
void print_instruction(const instr_trace_t& inst, int index) {
  ...
  std::cout << "  Branch Type: "
            << (inst.branch_type.empty() ? "(none)" : inst.branch_type)
            << std::endl;
  std::cout << "  Instruction Sync: " << (inst.inst_sync ? "true" : "false")
            << std::endl;
...
int main(int argc, char* argv[]) {
  ...
  while (arg_idx < argc) {
    ...
    if (arg == "--range") {
      if (arg_idx + 2 >= argc) {
        std::cerr << "Error: --range requires two arguments (START END)"
                  << std::endl;
        print_usage(argv[0]);
        return 1;
      }
  ...
  if (use_range && use_start_num) {
    std::cerr << "Error: Cannot use both --range and --start/--num together"
              << std::endl;
    print_usage(argv[0]);
    return 1;
  }
  ...
  if (use_start_num && (start_idx == -1 || num_instructions == -1)) {
    std::cerr << "Error: --start and --num must both be specified together"
              << std::endl;
    print_usage(argv[0]);
    return 1;
  }
  ...
  if (use_range) {
    ...
    if (print_start > print_end) {
      std::cerr << "Error: Invalid range: start (" << start_idx << ") > end ("
                << end_idx << ")" << std::endl;
      return 1;
    }
    std::cout << "\nPrinting instructions " << print_start << " to "
              << print_end << " (inclusive):" << std::endl;
  } else if (use_start_num) {
    print_start = std::max(0, start_idx);
    print_end = std::min((int)instructions.size() - 1,
                         print_start + num_instructions - 1);
    std::cout << "\nPrinting " << num_instructions
              << " instructions starting from " << print_start << ":"
              << std::endl;
  } else {
    // Default: print first 5 instructions
    print_start = 0;
    print_end = std::min(4, (int)instructions.size() - 1);
    std::cout << "\nPrinting first " << (print_end - print_start + 1)
              << " instructions (default):" << std::endl;
  }
